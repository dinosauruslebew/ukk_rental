...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // --- DATA KALENDER (Hanya tampilkan yang BELUM selesai atau dibatalkan) ---
        $items = OrderItem::with(['order.user'])
            ->where('tanggal_sewa', '>=', Carbon::now()->subMonths(3))
            ->whereHas('order', fn($q) => $q->whereNotIn('status', ['selesai', 'dibatalkan'])) // <--- PERBAIKAN FILTER: Lewati yang 'selesai'
            ->get();
