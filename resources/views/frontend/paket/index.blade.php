...
<style>
    ...
    /* Basic style for buttons for compatibility */
    .form-button {
    ...
    <div class="min-h-screen bg-gray-50 pt-24 pb-20 px-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex gap-2 md:gap-4 min-w-max">
                ...
                class="px-5 py-2 rounded-full text-sm font-medium border border-gray-400 whitespace-nowrap
                ...
                @foreach($kategori as $cat)
                    ...
                    class="px-5 py-2 rounded-full text-sm font-medium capitalize border border-gray-400 whitespace-nowrap
...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 items-stretch">
    ...
    {{-- PENENTUAN WARNA BERDASARKAN NAMA PAKET UTAMA --}}
    @php
        $namaUpper = strtoupper($item->nama_paket);
        if (str_contains($namaUpper, 'GRILL') || str_contains($namaUpper, 'BBQ')) {
            $badgeColor = 'bg-red-100 text-red-600';
            $iconColor = 'text-red-500';
        } elseif (str_contains($namaUpper, 'CHILL') || str_contains($namaUpper, 'SANTAI')) {
            $badgeColor = 'bg-blue-100 text-blue-600';
            $iconColor = 'text-blue-500';
        } else {
            // Default untuk CAMP/REGULER/STANDAR
    ...
    {{-- Card dengan tinggi minimal dan tombol di bawah --}}
    <div class="bg-white border border-gray-200 rounded-3xl shadow-lg hover:shadow-xl p-6 relative transition flex flex-col justify-between h-full">

        <div class="flex-grow">
            <!-- Badge Kategori -->
            <span class="text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-full w-fit mb-4 block {{ $badgeColor }}">
                PAKET LENGKAP
            </span>
            ...
            <div class="mb-6 pt-4 border-t border-gray-100">
                ...
                <ul class="space-y-1 text-sm text-gray-700">
                    ...
                    @foreach($item->items->take($maxDisplay) as $barangPaket)
                    <li class="flex items-start gap-2">
                        <i class="fa-solid fa-check {{ $iconColor }} mt-1"></i>
                        <span>
                    ...
                    @if ($totalItems > $maxDisplay)
                        <li class="text-xs text-gray-500 italic mt-2">
                            + {{ $totalItems - $maxDisplay }} item lainnya...
                        </li>
                    @endif
                </ul>
            </div>
        ...
        <a href="{{ route('frontend.paket.detail', $item->id_paket) }}"
            class="mt-auto block w-full text-center bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl py-3 transition shadow-md form-button">
            Pilih Paket Ini
        </a>
    ...
    @empty
        ...
        <div class="col-span-full flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900">Paket tidak ditemukan</h3>
            <p class="text-gray-500 mt-1">Coba pilih kategori lain atau reset filter.</p>
