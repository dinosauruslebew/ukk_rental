...
<style>
    ...
    .fc .fc-button {
        ...
        border: 1px solid #e5e7eb; /* Border lebih soft */
        color: #374151;
    ...
    /* HILANGKAN BORDER TABLE BIAR BERSIH (seperti di gambar) */
    ...
    .fc-day-selected .fc-daygrid-day-number {
        /* PERBAIKAN WARNA: Gunakan warna yang lebih netral tapi tegas */
...
{{-- PHP helper untuk badge status --}}
@php
    function getStatusBadgeClasses($status) {
        switch ($status) {
            case 'selesai': return 'bg-emerald-100 text-emerald-700 border-emerald-200';
            case 'dikonfirmasi': return 'bg-teal-100 text-teal-700 border-teal-200';
            case 'disewa': return 'bg-orange-100 text-orange-600 border-orange-200';
...
<div class="p-6 md:p-8 min-h-screen bg-[#f7fafc]">
    ...
    <!-- Statistik Cards (Layout 4 kolom) -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-200">
            <div class="flex items-center justify-between">
                <div><p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Barang</p><p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $totalBarang }}</p></div>
                <div class="w-12 h-12 rounded-full bg-teal-50 text-teal-600 flex items-center justify-center"><i class="fa-solid fa-box text-xl"></i></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-200">
            <div class="flex items-center justify-between">
                <div><p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Total User</p><p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $totalUsers }}</p></div>
                <div class="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center"><i class="fa-solid fa-users text-xl"></i></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-200">
            <div class="flex items-center justify-between">
                <div><p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Rental Aktif</p><p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $totalRentalAktif }}</p></div>
                <div class="w-12 h-12 rounded-full bg-orange-50 text-orange-500 flex items-center justify-center"><i class="fa-solid fa-clock text-xl"></i></div>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-200">
            <div class="flex items-center justify-between">
                <div><p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Stok Habis</p><p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $barangTidakTersedia }}</p></div>
                <div class="w-12 h-12 rounded-full bg-red-50 text-red-500 flex items-center justify-center"><i class="fa-solid fa-triangle-exclamation text-xl"></i></div>
            </div>
        </div>
    </div>

    <!-- Main Layout (1/3 Kalender & Details | 2/3 Charts & Table) -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">

        <!-- KOLOM KIRI (1/3): Kalender & Detail -->
        <div class="lg:col-span-1 space-y-6">
            ...
            <div id="dayDetails" class="hidden bg-white rounded-2xl shadow-xl border border-gray-100 p-5 animate-fade-in-up">
        ...
        dateClick: function(info) {
            ...
            if (selectedEvents.length > 0) {
                dayEventsList.innerHTML = selectedEvents.map(e => {
                    ...
                    // Penentuan URL Detail (Diasumsikan ID Order ada di props.order_id)
                    const detailUrl = `/admin/orders/${props.order_id ?? e.id}`; 
                    
                    // --- HTML Output ---
                    return `
                        <div class="flex items-start gap-3 p-3 bg-white rounded-xl border border-gray-100 hover:border-gray-300 transition group">
